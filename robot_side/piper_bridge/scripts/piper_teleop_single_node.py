#!/usr/bin/env python3
# -*-coding:utf8-*-
# 本文件为控制单机械臂节点，用于teleop示教和robot运动控制
from typing import (
    Optional,
)
import rospy
...
class C_PiperRosNode():
    """机械臂ros节点
    """
    def __init__(self, arm_type: str = 'teleop') -> None:
        """
        Args:
            arm_type: 机械臂类型，'teleop' 或 'robot'
        """
        check_ros_master()
        if arm_type not in ['teleop', 'robot']:
            raise ValueError("arm_type must be 'teleop' or 'robot'")
        self.arm_type = arm_type
        # 只在节点未初始化时初始化
        if not rospy.get_node_uri():
            rospy.init_node('piper_teleop_single_node', anonymous=True)
        
        # 外部param参数
        # can路由名称，根据arm_type分别获取teleop和robot的port
        if arm_type == 'teleop':
            default_port = "can0"
            param_name = '~teleop_can_port'
        else:
            default_port = "can1"
            param_name = '~robot_can_port'
        ...
        if self.arm_type == 'robot':
            self.joint_state_source_topic = '/teleop/arm_teleop/joint_states_single'
        else:
            self.joint_state_source_topic = self.topic_prefix + 'joint_ctrl_single'
        ...
        # service
        self.enable_service = rospy.Service(self.topic_prefix + 'enable_srv', Enable, self.handle_enable_service)  # 创建enable服务
        self.__enable_flag = False
        self.gripper_service = rospy.Service(self.topic_prefix + 'gripper_srv', Gripper, self.handle_gripper_service)  # 创建gripper服务
        self.stop_service = rospy.Service(self.topic_prefix + 'stop_srv', Trigger, self.handle_stop_service)  # 创建stop服务
        self.reset_service = rospy.Service(self.topic_prefix + 'reset_srv', Trigger, self.handle_reset_service)  # 创建reset服务
        self.go_zero_service = rospy.Service(self.topic_prefix + 'go_zero_srv', GoZero, self.handle_go_zero_service)  # 创建reset服务
        ...
        # 对于teleop类型，进入拖动示教模式
        if self.arm_type == 'teleop':
            rospy.loginfo("Entering drag teaching mode for teleop arm...")
            # 进入拖动示教模式: grag_teach_ctrl = 0x01
            self.piper.MotionCtrl_1(0x00, 0x00, 0x01)
            rospy.loginfo("Teleop arm entered drag teaching mode.")
    ...
    def SubPosThread(self):
        ...
        """
        rospy.Subscriber(self.topic_prefix + 'pos_cmd', PosCmd, self.pos_callback, queue_size=1, tcp_nodelay=True)
    ...
    def SubJointThread(self):
        ...
        """
        rospy.Subscriber(self.joint_state_source_topic, JointState, self.joint_callback, queue_size=1, tcp_nodelay=True)
        rate = rospy.Rate(10)  # 10 Hz
        while not rospy.is_shutdown():
            rate.sleep()
...
if __name__ == '__main__':
    try:
        ...
        # 创建teleop和robot两个机械臂实例
        piper_teleop = C_PiperRosNode(arm_type='teleop')
        piper_robot = C_PiperRosNode(arm_type='robot')
        
        # 为每个机械臂启动发布线程
        teleop_pub_thread = threading.Thread(target=piper_teleop.Pubilsh)
        robot_pub_thread = threading.Thread(target=piper_robot.Pubilsh)
        
        teleop_pub_thread.daemon = True
        robot_pub_thread.daemon = True
        
        teleop_pub_thread.start()
        robot_pub_thread.start()
