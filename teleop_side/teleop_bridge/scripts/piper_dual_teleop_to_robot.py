...
class C_PiperRosNode:
    ...
    def __init__(self, arm_type: str = "left") -> None:
        ...
        self.joint_pub = rospy.Publisher(
            self.topic_prefix + "joint_states_single", JointState, queue_size=1
        )
        self.arm_status_pub = rospy.Publisher(
            self.topic_prefix + "arm_status", PiperStatusMsg, queue_size=1
        )
        self.end_pose_pub = rospy.Publisher(
            self.topic_prefix + "end_pose", PoseStamped, queue_size=1
        )
        self.end_pose_euler_pub = rospy.Publisher(
            self.topic_prefix + "end_pose_euler", PiperEulerPose, queue_size=1
        )
        ...
        self.block_arm_service = rospy.Service(
            self.topic_prefix + "block_arm", SetBool, self.handle_block_arm_service
        )
    ...
    def PublishArmJointAndGripper(self):
        ...
        joint_0: float = (
            self.piper.GetArmJointMsgs().joint_state.joint_1 / 1000
        ) * 0.017444
        joint_1: float = (
            self.piper.GetArmJointMsgs().joint_state.joint_2 / 1000
        ) * 0.017444
        joint_2: float = (
            self.piper.GetArmJointMsgs().joint_state.joint_3 / 1000
        ) * 0.017444
        joint_3: float = (
            self.piper.GetArmJointMsgs().joint_state.joint_4 / 1000
        ) * 0.017444
        joint_4: float = (
            self.piper.GetArmJointMsgs().joint_state.joint_5 / 1000
        ) * 0.017444
        joint_5: float = (
            self.piper.GetArmJointMsgs().joint_state.joint_6 / 1000
        ) * 0.017444
        joint_6: float = (
    ...
    def handle_block_arm_service(self, req):
        ...
        rospy.loginfo(
            f"Returning BlockArmResponse: {response.success}, {response.message}"
        )
        rospy.loginfo(f"-----------------------BLOCK_ARM---------------------------")
        return response
