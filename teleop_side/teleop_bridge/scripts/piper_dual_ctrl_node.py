...
class C_PiperRosNode():
    ...
    def __init__(self, arm_side: str = 'left') -> None:
        ...
        if not rospy.get_node_uri():
            rospy.init_node('piper_ctrl_dual_node', anonymous=True)
        # 外部param参数
        # can路由名称，根据arm_side分别获取left和right的port
        if arm_side == 'left':
            default_port = "can0"
    ...
    def SubJointThread(self):
        ...
        """
        topic_name = self.topic_prefix + 'joint_states_single'
        topic_name_cmd = topic_name.replace('robot', 'teleop')
        rospy.loginfo("Subscribing to: %s", topic_name_cmd)
        rospy.Subscriber(topic_name_cmd, JointState, self.joint_callback, queue_size=1, tcp_nodelay=True)
...
if __name__ == '__main__':
    try:
        ...
        # 创建左右两个机械臂实例
        piper_left = C_PiperRosNode(arm_side='left')
        piper_right = C_PiperRosNode(arm_side='right')
