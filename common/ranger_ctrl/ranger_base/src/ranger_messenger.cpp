...
namespace westonrobot {
    ...
    void RangerROSMessenger::SetupSubscription() {
        ...
        rs_state_pub_ = nh_->advertise<ranger_msgs::RsStatus>("rs_state", 10);
    ...
    void RangerROSMessenger::PublishStateToROS() {
        ...
        // publish actuator state
        {
            ...
            actuator_msg.states[0].motor.motor_angles =
                actuator_state.motor_angles.angle_5;
            actuator_msg.states[1].motor.motor_angles =
                actuator_state.motor_angles.angle_6;
            actuator_msg.states[2].motor.motor_angles =
                actuator_state.motor_angles.angle_7;
            actuator_msg.states[3].motor.motor_angles =
                actuator_state.motor_angles.angle_8;

            actuator_msg.states[0].motor.motor_speeds =
                actuator_state.motor_speeds.speed_1;
            actuator_msg.states[1].motor.motor_speeds =
                actuator_state.motor_speeds.speed_2;
            actuator_msg.states[2].motor.motor_speeds =
                actuator_state.motor_speeds.speed_3;
            actuator_msg.states[3].motor.motor_speeds =
                actuator_state.motor_speeds.speed_4;
    ...
    void RangerROSMessenger::TwistCmdCallback(
        ...
        case MotionState::MOTION_MODE_PARALLEL: {
            ...
            // Note: Do NOT flip steer_cmd based on signbit(x)
            // atan(y/x) already produces the correct steering angle for all quadrants
            // When x < 0, the negative velocity combined with the unflipped angle
            // produces the correct movement direction
            ...
            if (msg->linear.x == 0.0 && msg->linear.y != 0.0) {
                // Pure lateral movement (x=0): steer_cmd sign should match y direction
                // Use positive velocity with appropriate steering angle
                steer_cmd = msg->linear.y >= 0 ? std::abs(steer_cmd) : -std::abs(steer_cmd);
                vel = 1.0;
            }
            else {
